...
describe("Annuity Contract", () => {
  ...
  it("Should create a agreement", async () => {
    ...
    //boorower repays the loan
    tx5 = await USDC.connect(borrower).approve(
      Annuity.address,
      totalPayBackAmt
    );
    tx5.wait();

    tx5 = await Annuity.connect(borrower).repay(1, totalPayBackAmt);
    temp = await tx5.wait();

    //print reapy balance of lender and borrower
    console.log(
      "After repay balance of lender is %d and balance of borrower is %d",
      await USDC.balanceOf(lender.address),
      await USDC.balanceOf(borrower.address)
    );

    id = temp.events?.filter((x: any) => {
      return x.event === "RepaidAgreement";
    })[0].args?.agreementId;
    let repaidAmount = temp.events?.filter((x: any) => {
      return x.event === "RepaidAgreement";
    })[0].args?.amount;

    expect(id).to.equal(1);
    expect(repaidAmount).to.equal(totalPayBackAmt);
  });
  it("Borrower should repay the loan", async () => {
    
    const id = temp.events?.filter((x: any) => {
      return x.event === "RepaidAgreement";
    })[0].args?.agreementId;
    let repaidAmount = temp.events?.filter((x: any) => {
      return x.event === "RepaidAgreement";
